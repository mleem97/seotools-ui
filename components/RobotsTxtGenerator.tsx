...
export default function RobotsTxtGenerator({
  ...
  const [logs, setLogs] = useState<{ id: string; time: string; text: string }[]>(() => {
    ...
    return [
      { id: "log-init", time, text: "System initialized. Welcome to Pinkbot Control Room." },
      { id: "log-preset", time, text: "Loaded default crawl policy rules." }
    ];
  });
  ...
  const addLog = (text: string) => {
    ...
    setLogs((prev) => [
      ...prev,
      { id: `log-${Date.now()}-${Math.random()}`, time, text }
    ].slice(-4)); // limit to last 4 console messages
  };
...
Once created, explain how I can import and render <RobotsTxtGenerator /> in my layout or page files.`;
    ...
    addLog("Copied installer CLI shell prompt");
    setTimeout(() => setPromptCopied(false), 2000);
  };

  // Auto-generate robots.txt content
  const generatedRobotsTxt = useMemo(() => {
    let output = "# Generated by Pinkbot Robots.txt Generator\n";
  ...
  // Track validation logging
  useEffect(() => {
    if (testPath !== "/" || testUserAgent !== "*") {
      addLog(`Validation test requested for path: "${testPath}" (${testUserAgent})`);
    }
  }, [testPath, testUserAgent]);
  ...
  const loadPreset = (presetKey: keyof typeof PRESETS) => {
    ...
    addLog(`Applied rules preset config: "${preset.name}"`);
  };
  ...
  const addSitemap = (e: React.FormEvent) => {
    ...
    if (!sitemaps.includes(newSitemap.trim())) {
      const added = newSitemap.trim();
      setSitemaps([...sitemaps, added]);
      addLog(`Added sitemap URL: "${added}"`);
    }
    setNewSitemap("");
  };

  const removeSitemap = (index: number) => {
    const removed = sitemaps[index];
    setSitemaps(sitemaps.filter((_, idx) => idx !== index));
    addLog(`Removed sitemap URL: "${removed}"`);
  };
  ...
  const addGroup = (ua = "*") => {
    ...
    addLog(`Created empty group for User-agent: "${ua}"`);
  };
  ...
  const removeGroup = (groupId: string) => {
    ...
    if (targetGroup) {
      addLog(`Removed rule group for User-agent: "${targetGroup.userAgent}"`);
    }
  };
    ...
    setGroups(
      groups.map((g) => {
        if (g.id === groupId) {
          ...
          const validated = isNaN(val as number) ? undefined : val;
          return { ...g, crawlDelay: validated };
        }
        return g;
      })
    );
  ...
  const addRule = (groupId: string) => {
    ...
    addLog("Appended blank directive row");
  };
  ...
  const addAiBlocks = () => {
    ...
    addLog("Blocked all core AI crawler bots (* -> Disallow: /)");
  };
  ...
  const copyToClipboard = () => {
    ...
    setCopied(true);
    addLog("Copied generated robots.txt file code");
  ...
  const downloadRobotsTxt = () => {
    ...
    addLog("Printed file: Downloaded robots.txt locally");
  };
  ...
  const handleImport = () => {
    ...
    try {
      ...
      setImportText("");
      addLog("Successfully parsed and loaded raw robots.txt input");
      setActiveTab("edit");
    } catch (e: any) {
    ...
    <div className="w-full max-w-6xl mx-auto rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-100 shadow-2xl overflow-hidden font-sans">
      ...
      <div className="bg-zinc-950 border-b border-zinc-800 px-4 py-2 flex items-center justify-between font-mono text-[10px] tracking-wide text-zinc-500">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-pink-500/20 flex items-center justify-center">
        ...
        <div className="lg:col-span-7 p-6 md:p-8 flex flex-col space-y-6">
          ...
          <div className="flex flex-col space-y-2">
            ...
            <p className="text-xs text-zinc-400">
              Baue deine robots.txt Schritt für Schritt. Pinkbot zeigt dir direkt, welche Regeln entstehen und wie Crawler sie lesen würden.
            </p>
          </div>
            ...
            <div className="space-y-6 flex-1">
              ...
              <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800 space-y-3">
                ...
                <div className="pt-2 border-t border-zinc-800/80 flex justify-between items-center text-[10px] text-zinc-500">
                  <span>Sollen AI-Crawler erst mal ausgesperrt werden?</span>
                  <button
                    onClick={addAiBlocks}
                          ...
                          <select
                            onChange={(e) => {
                              if (e.target.value) {
                                ...
                                addLog(`Targeted group User-agent changed to: "${e.target.value}"`);
                                e.target.value = "";
                              }
                            }}
                              ...
                              <div key={rule.id} className="flex gap-2 items-center">
                                <select
                                  ...
                                  onChange={(e) => {
                                    ...
                                    addLog(`Changed rule type to: ${e.target.value}`);
                                  }}
              ...
              <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 space-y-2.5">
                <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-wider text-zinc-500">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-pink-400 font-bold">Pinkbot says:</span>
                  </div>
                  {testPathRules.isFallback && (
                    <span className="text-zinc-700 font-semibold">Fallback * matched</span>
                  )}
                </div>
                <div className="font-mono text-xs text-zinc-300 leading-relaxed border-t border-zinc-900 pt-2.5">
                  {testPathRules.allowed ? (
                    <span>
                      🟢 Access <strong className="text-green-400">ALLOWED</strong>. {testPathRules.reason}
                    </span>
                  ) : (
                    <span>
                      🔴 Access <strong className="text-red-400">BLOCKED</strong>. {testPathRules.reason}
                    </span>
                  )}
                </div>
              </div>
...
Sitemap: https://example.com/sitemap.xml`}
        ...
        <div className="lg:col-span-5 p-6 md:p-8 bg-zinc-950/10 flex flex-col space-y-6">
          ...
          {/* Console Log Panel */}
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 flex flex-col space-y-3">
            <div className="flex items-center justify-between text-[10px] font-mono font-semibold tracking-wider text-zinc-500 uppercase">
              <span>SYSTEM LOG</span>
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-pink-500 animate-pulse" />
                <span className="text-pink-500">Pinkbot Active</span>
              </div>
            </div>
            
            <div className="bg-zinc-900/40 rounded-lg p-3 border border-zinc-800/60 font-mono text-[9px] text-zinc-400 space-y-1.5 overflow-hidden">
              {logs.map((log) => (
